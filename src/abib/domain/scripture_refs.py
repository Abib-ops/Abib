...
def normalize_reference(reference_text: str, current_book: int = 0, current_chapter: int = 0) -> str:
    ...
    reference_text = fcs.clean_chap_prefix(reference_text)
    ...
    # Roman numeral check for specific book abbreviations
    roman_book: list[str] = ['i', 'l', 'c', 'd', 'm']
    ...
    # If it's just a number, it's a verse in the current chapter
    if reference_text.isdigit():
        return reference_text

    # Basic regex to see if it already looks like B.C.V
    ...
    input_book = matched.group(1)
    input_chapter = matched.group(2)
    input_verse = matched.group(3)

    book_str: str = str(input_book).strip() if input_book else str(current_book)
    chapter_val: int = int(input_chapter) if input_chapter else int(current_chapter) + 1
    verse_val: int = int(input_verse) if input_verse else 1
