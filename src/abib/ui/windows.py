...
class SecondaryWindow(NoZoomDialog):
    def __init__(
        ...
        try:
            self.fontsize = self.settings_service.get_devotional_font_size()
...
class AboutWindow(QMainWindow):
    def __init__(self, version_text: str, settings_service: SettingsService | None = None):
        super().__init__()
    ...
    def about() -> tuple[str, str]:
        ...
        content: str = ""
        file_path = sh.base_dir / "ABOUT.txt"
        try:
            with open(file_path, "r", encoding="utf-8") as file_about:
                content = file_about.read()
        except FileNotFoundError:
            content = f"ABOUT.txt file not found (looked in {file_path})."
